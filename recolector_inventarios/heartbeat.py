...
def _register_instances(instances: list[str], interval: float) -> None:
    ...
    for name in instances:
        ServiceRegistration.objects.get_or_create(
            name=name,
            defaults={
                'description': f'Instancia simulada del Recolector de Inventarios ({name})',
...
class HeartbeatSender:
    ...
    def start(cls) -> None:
        ...
        instances: list[str] = getattr(
            settings, 'INVENTORY_COLLECTOR_INSTANCES',
            ['recolector_inventarios_1', 'recolector_inventarios_2'],
        )
        ...
        t = threading.Thread(
            ...
            daemon=True,
            name='HeartbeatSender-Recolector',
        )
        t.start()
        cls._thread = t
        logger.info(
            "HeartbeatSender (recolector) arrancado — instancias: %s, intervalo: %.1fs",
            instances, interval,
        )
